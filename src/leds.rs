...
use esp_hal::prelude::{_embedded_hal_digital_v2_OutputPin, _esp_hal_ledc_channel_ChannelHW, _esp_hal_ledc_channel_ChannelIFace, _esp_hal_ledc_timer_TimerIFace, _fugit_RateExtU32};
use esp_hal::timer::TimerGroup;
use esp_println::println;
use picoserve::{Config, listen_and_serve, Router};
...
pub fn setup_leds(value: &'static ValueSynchronizer<MAX_LISTENERS, NoopRawMutex, InputMessage>, red: GpioPin<Unknown, PIN_RED>, blue: GpioPin<Unknown, PIN_BLUE>, clocks: &'static Clocks, ledc: esp_hal::peripherals::LEDC, spawner: Spawner) {
    let red: &'static mut _ = make_static!(red.into_push_pull_output());
    let blue: &'static mut _ = make_static!(blue.into_push_pull_output());

    let ledc: &'static mut LEDC = make_static!(LEDC::new(
        ledc,
        &clocks,
    ));
    ...
    let mut red_channel =
        ledc.get_channel(channel::Number::Channel0, red);
    red_channel.configure(channel::config::Config {
        timer,
        duty_pct: 0,
        pin_config: PinConfig::PushPull,
    }).unwrap();

    let mut blue_channel =
        ledc.get_channel(channel::Number::Channel1, blue);
    blue_channel.configure(channel::config::Config {
        timer,
        duty_pct: 0,
        pin_config: PinConfig::PushPull,
    }).unwrap();
